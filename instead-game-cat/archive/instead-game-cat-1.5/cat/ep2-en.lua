...
moika = room {
	nam = 'dish washing',
	enter = function()
		return cat('I take the tray to the dish washing area.^^', goto('kitchen')), false;
	end
};
...
[[— Security level 4. Hall 2. So, be there! It's a good opportunity to get close to... — Ilya gave a look at one of the portraits on the wall. — Oh, I almost forgot! — he gave me a piece of white plastic — Well, see ya!... — Uhhh...]],[[inv():add('invite');return goto('eating');]]), 
	}
};
kitchen = room {
	nam = 'canteen',
	...
	act = function(s, w)
		...
		if w == 3 and not have('food') then
			return cat([[I joined the queue... I took a tray, cutlery and wipes. Time flows painfully slowly. At last I make an order...^^]], goto('povardlg'));
		end
	end,
...
hall42 = room {
	...
	enter = function(s, f)
		...
		if not me()._brit or not galstuk._wear then
			return cat(
'I try to enter the hall, but a man in a uniform stops me. I read <<SECURITY>> on his label. He holds a shotgun.^^', goto('facectrl')), false;
		end
		facectrl:poff(1, 5);
		facectrl:pon(6);
